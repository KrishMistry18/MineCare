 ...
 * Exact visual match to Lovable reference implementation with RBAC-aware navigation.
 */
...
export const Sidebar: React.FC<SidebarProps> = ({ currentRoute, onRouteChange }) => {
  ...
  const baseNavItems: { route: NavRoute; label: string; icon: React.ElementType; badge?: string | null }[] = [
    {
      route: '/',
      label: 'Dashboard',
  ...
  return (
    <aside className="w-60 bg-[#080d16] border-r border-[#151f30] flex flex-col justify-between shrink-0 select-none h-screen sticky top-0">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                  isActive
                    ? 'bg-[#131d2c] text-white font-semibold'
                    : 'text-slate-400 hover:text-slate-200 hover:bg-[#0c1420]'
                }`}
              >
                <div className="flex items-center space-x-3">
                  <Icon className={`w-4 h-4 ${isActive ? (item.route === '/admin' ? 'text-purple-400' : 'text-cyan-400') : 'text-slate-400'}`} />
      ...
      {/* Bottom Area: Authenticated User Menu + Source Indicator */}
      <div className="space-y-1 pb-2">
        <UserMenu />

        <div className="p-3 mx-3 rounded-lg bg-[#0d1420] border border-[#1a2538] text-xs space-y-1.5">
